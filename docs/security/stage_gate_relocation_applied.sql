-- AOS STAGE GATE RELOCATION APPLIED
-- Function: public.enforce_job_stage_transition()
-- Applied rule:
-- - INTAKE may move to INSPECTION without recorded inspection result.
-- - INSPECTION may not move into production without recorded inspection result.
-- - READY_FOR_INVOICE verified-report gate remains intact.
-- - THIRD_PARTY_QC completed-visit gate remains intact.
-- Validation:
-- - SQL validation passed.
-- - App validation passed: INTAKE -> INSPECTION allowed.
-- - App validation passed: INSPECTION -> production blocked without result.
...
declare
    ...
    inspection_result_exists boolean;
...
begin
    ...
    ------------------------------------------------------------
    -- INSPECTION RESULT ENFORCEMENT
    -- INTAKE may move to INSPECTION.
    -- INSPECTION may not move into production / routing decision
    -- without a recorded in-house inspection result.
    ------------------------------------------------------------
    if old.internal_status = 'INSPECTION' then
        select exists (
            select 1
            from job_inspection_sessions jis
            join job_inspection_results jir
                on jir.session_id = jis.id
            where jis.job_id = old.id
              and jis.stage_at_time = 'INSPECTION'
              and jir.result in ('PASS', 'FAIL')
        ) into inspection_result_exists;

        if not inspection_result_exists then
            raise exception 'Cannot leave INSPECTION without recorded inspection result.';
        end if;
    end if;
